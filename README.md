...
| 传输 | 原生 TLS；WebSocket over TLS（WSS）；VLESS + REALITY（服务端，RAW TCP） |
| 出站 | Direct TCP/UDP |
...
- 内置客户端只实现 Trojan，不实现 VLESS 客户端，也不实现 REALITY 客户端。
- VLESS 服务端有两种入口：经典 VLESS over WSS（需要 `[websocket]`），或 VLESS + REALITY（RAW TCP）；两者都不能与 `[trojan]` 同时配置。
- REALITY 仅实现服务端，仅与 `[vless]` 搭配，且不能与 `[tls]`、`[websocket]`、`[mux]` 同时配置。
- REALITY 第一阶段不实现 fingerprint-probe、XTLS Vision（`flow` 必须留空）、ML-DSA 后量子签名。
...
完整的测试矩阵、基线格式、CI 门禁和真实浏览器采集边界见 [TLS 指纹抓包回归测试方案](docs/tls-fingerprint-regression.md)。
...
VLESS 可运行在 strict WebSocket 入口之上（VLESS over WSS），也可运行在 REALITY 入口之上（VLESS + REALITY，RAW TCP）。当前未实现 flow / XTLS Vision 与 VLESS 客户端：请求头携带 addons 时会被拒绝，因此第三方客户端必须把 `flow` 留空。

### REALITY（服务端）

REALITY 是 XTLS 生态用于削弱服务端 TLS 可识别性的传输方案。本项目第一阶段实现 **VLESS + REALITY over RAW TCP 服务端**，可被 Xray/sing-box 客户端连入。

握手与鉴权流程（服务端视角）：

1. 在交给 TLS 库之前，先有界、带超时地预读完整 ClientHello，从而精确控制后续用于 AES-GCM 的 AAD。
2. 校验客户端通告 TLS 1.3、SNI 命中 `server_names`、提供 X25519 key share。
3. 用服务端 X25519 私钥与客户端 key share 做 ECDH，经 `HKDF-SHA256(salt = client_random[..20], info = "REALITY")` 派生 `auth_key`。
4. 用 `auth_key` 做 AES-256-GCM，AAD 为 `session_id` 清零后的 ClientHello，解出 16 字节明文，校验 `short_id` 与可选的 `max_time_diff_secs` 时间窗口。
5. 鉴权通过：以一张每连接伪造的 Ed25519 临时证书（签名字段替换为 `HMAC-SHA512(auth_key, ed25519_pub)`）完成真实 TLS 1.3 握手，再把明文流交给 VLESS。BoringSSL 用真实 Ed25519 私钥签 CertificateVerify，不校验叶证书自签名。
6. 鉴权失败（SNI/short_id/时间/认证不符，或非 TLS 1.3）：连接真实 `target`，回放已读取的字节并透明转发，使探测者看到真实站点的 TLS 行为。

与内置 `[fallback]` 的区别：`[fallback]` 是 TLS 握手完成后对错误 HTTP 请求返回的静态页面；REALITY 的失败转发是在 TLS 之前把整条连接原样转交给真实 `target`，二者层级不同。

配置要求与注意事项：

- `private_key` 是 `xray x25519` 生成的 X25519 私钥（base64），客户端配置对应公钥。
- `server_names` 必须是合法 DNS 主机名，且应被 `target` 站点证书 SAN 覆盖。
- `short_ids` 为 hex、偶数长度、最多 8 字节；`""` 允许不带 short ID 的客户端。
- `target` 必须真实可达并支持 TLS 1.3；建议同时支持 HTTP/2。
- 客户端 `flow` 必须留空；配置 `xtls-rprx-vision` 会因 VLESS addons 不被支持而失败。
- 完整模板见 [`config/server-reality.toml`](config/server-reality.toml)。

> [!WARNING]
> REALITY 的伪造证书与 AAD 必须与 Xray 客户端逐字节兼容。本实现已通过密钥派生、AES-GCM、ClientHello 解析和证书 DER 的单元测试，但跨实现互通仍需用真实 `xray`/`sing-box` 客户端验证后再投入生产。
...
完整模板位于 [`config/server.toml`](config/server.toml)、[`config/server-reality.toml`](config/server-reality.toml)、[`config/client.toml`](config/client.toml) 和 [`config/forward.toml`](config/forward.toml)。当前主程序实现的运行模式是 `server` 和 `client`。
...
Debian 11 官方仓库提供的 CMake 3.18 不能构建当前 `boring-sys` 内含的 BoringSSL。发布流程会在 Debian 11 容器内通过固定版本的 Python CMake wheel 提供 CMake 3.31.6，使 GNU 产物保持 glibc 2.31 基线，同时满足 BoringSSL 的最低构建要求。
...
CI 发布流程分别构建 client/server，并生成 Linux glibc、完全静态的 Linux musl、Windows 和 macOS 产物。GNU 与 musl 压缩包都会在干净 Debian 11 容器中解压并执行 `--version`；所有平台产物齐全后才统一创建 Release，避免部分 job 失败时发布不完整版本。BoringSSL 构建时间明显长于纯 Rust 依赖，首次编译属于正常现象。
...
“BoringSSL 静态链接”不等于 GNU 目标完全静态。GNU 产物仍受 glibc 最低版本影响，因此当前发布流程在 Debian 11 中构建，将最低 glibc 基线限制为 2.31。musl 产物额外启用静态 CRT，并拒绝任何仍包含 ELF program interpreter 的构建结果。

安装脚本会优先为 Alpine 选择 `linux-musl-*`，为 Debian/Ubuntu 等发行版选择 `linux-*` GNU 资产；首选产物缺失或无法执行时，会自动尝试另一种 Linux 运行时产物，并输出 ELF loader/动态库诊断。发布验证至少覆盖：
